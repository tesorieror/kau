app.controller('DescriptionDialogCtrl', function($scope, $http, $q, $modalInstance, $log, indicator, dataStoreService) {
	...
	function buildColumnChart() {
		...
		for (yr in $scope.data) {
			...
			if (yr >= $scope.fromYear && yr <= $scope.toYear) {
				// var val = ($scope.indicator.filters.length > 1) ?
				// value[$scope.indicator.filters[0]][$scope.indicator.filters[1]] :
				// value[$scope.indicator.filters[0]];
				var val = ($scope.indicator.filter == null) ? value[$scope.indicator.subsubcategory] : value[$scope.indicator.subsubcategory][$scope.indicator.filter];
				total = total + val;
			}
		}
		...
		for (yr in $scope.data) {
			...
			if (yr >= $scope.fromYear && yr <= $scope.toYear) {
				last = actual;
				// actual = ($scope.indicator.filters.length > 1) ?
				// value[$scope.indicator.filters[0]][$scope.indicator.filters[1]] :
				// value[$scope.indicator.filters[0]];
				actual = ($scope.indicator.filter == null) ? value[$scope.indicator.subsubcategory] : value[$scope.indicator.subsubcategory][$scope.indicator.filter];
	...
	dataStoreService.getYears().then(function(years) {
		...
		$q.all([ dataStoreService.getData(indicator), dataStoreService.getDescriptionFor(indicator) ]).then(function(result) {
			var data = result[0];
